...
const CDN = `${(process.env.VITE_BUNNY_CDN_URL || process.env.BUNNY_CDN_URL || '').replace(/\/$/, '')}/`;
...
const numericFields = new Set(['episode_number','duration_seconds','sort_order','price','duration_days','plan_id','amount','video_id','current_position','watched_duration','duration','watch_seconds','campaign_id','show_after_seconds','frequency_hours','reward_amount','coins','resource_id']);
...
function defaultInsert(resource, body) {
  ...
  if (resource === 'videos') {
    ...
    // Optional columns are added only when present so old deployments / stale
    // PostgREST schema caches do not block core video saves.
    if (body.age_rating) video.age_rating = body.age_rating;
    if (body.publish_at) video.publish_at = body.publish_at;
    return video;
  }
  ...
  if (resource === 'watch_history') return { user_id: body.user_id || '', video_id: Number(body.video_id || 0), current_position: Number(body.current_position || body.watched_duration || 0), duration: Number(body.duration || 0), completed: !!body.completed, updated_at: now };
...
function stripMissingSchemaColumn(payload, error) {
  const message = `${error?.message || ''} ${error?.details || ''} ${error?.hint || ''}`;
  const match = message.match(/(?:find|column)\s+the\s+([a-zA-Z0-9_]+)\s+column|['"]([a-zA-Z0-9_]+)['"]\s+column|column\s+['"]?([a-zA-Z0-9_]+)['"]?/i);
  const missing = match?.[1] || match?.[2] || match?.[3];
...
async function insertWithSchemaFallback(resource, payload) {
  let result = await supabase.from(resource).insert(payload).select().single();
  if (result.error?.code === 'PGRST204') {
    const stripped = stripMissingSchemaColumn(payload, result.error);
    if (stripped) {
      console.warn(`[api] Supabase schema cache missing optional column "${stripped.missing}" on ${resource}; retrying without it.`);
      result = await supabase.from(resource).insert(stripped.payload).select().single();
    }
  }
  return result;
}

async function updateWithSchemaFallback(resource, id, payload) {
  let result = await supabase.from(resource).update(payload).eq('id', id).select().single();
  if (result.error?.code === 'PGRST204') {
    const stripped = stripMissingSchemaColumn(payload, result.error);
    if (stripped) {
      console.warn(`[api] Supabase schema cache missing optional column "${stripped.missing}" on ${resource}; retrying update without it.`);
      result = await supabase.from(resource).update(stripped.payload).eq('id', id).select().single();
    }
  }
  return result;
}
...
async function handleJsonImport(req, res) {
  ...
  if (error) throw error;
  const auditResult = await supabase.from('audit_logs').insert({ actor: 'admin', action: 'json_import', resource, metadata: { count: safeRows.length } });
  if (auditResult.error) console.warn('Audit log insert failed:', auditResult.error.message);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await insertWithSchemaFallback(resource, defaultInsert(resource, body));
      if (error) throw error;
      const auditResult = await supabase.from('audit_logs').insert({ actor: body.actor || 'system', action: 'create', resource, resource_id: data?.id || null, metadata: { via: 'catch_all_api' } });
      if (auditResult.error) console.warn('Audit log insert failed:', auditResult.error.message);
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await updateWithSchemaFallback(resource, id, set);
      if (error) throw error;
      const auditResult = await supabase.from('audit_logs').insert({ actor: updates.actor || 'admin', action: 'update', resource, resource_id: Number(id), metadata: { fields: Object.keys(set) } });
      if (auditResult.error) console.warn('Audit log insert failed:', auditResult.error.message);
      return res.status(200).json(data);
    }
    ...
    if (req.method === 'DELETE') {
      ...
      if (error) throw error;
      const auditResult = await supabase.from('audit_logs').insert({ actor: 'admin', action: 'delete', resource, resource_id: id ? Number(id) : null, metadata: { user_id, video_id } });
      if (auditResult.error) console.warn('Audit log insert failed:', auditResult.error.message);
