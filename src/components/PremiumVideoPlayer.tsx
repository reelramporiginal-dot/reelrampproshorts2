...
import {
  ...
  BarChart2, Zap, Shield, RefreshCcw, Ban, ChevronRight, Info
} from 'lucide-react';
...
type GatewayConfig = {
  ...
  cashfreeLink?: string; // <-- NEW: Cashfree payment page link
};
...
// Used by both locked Player screen and Plans page
// Returns the best active gateway from payment settings
const getActiveGateway = (payment: PaymentSettings): GatewayConfig | null =>
  payment.gateways.find(g => g.enabled && g.isDefault) ||
  payment.gateways.find(g => g.enabled) ||
  null;

// Opens payment based on gateway type
// For Cashfree: opens the cashfreeLink in a new tab
// For Razorpay/UPI: handled inside Plans component
const triggerCashfreePayment = (payment: PaymentSettings) => {
  const gw = getActiveGateway(payment);
  if (gw?.type === 'Cashfree' && gw.cashfreeLink) {
    window.open(gw.cashfreeLink, '_blank');
    return true;
  }
  return false;
};
...
function Provider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // ── FIX: Restore saved display name from localStorage ──
      if (next.users?.[0]) {
        const savedName = localStorage.getItem('rr_display_name');
        const u = next.users[0] as UserRow;
        if (savedName && !u.email) u.display_name = savedName;
        setUser(u);
      } else {
        // Create new user with saved name if exists
        const savedName = localStorage.getItem('rr_display_name') || `Viewer ${guestId.slice(-4)}`;
        const cr = await fetch('/api/users', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ guest_id: guestId, display_name: savedName, email: '', role: 'viewer' })
        });
...
function Player({ video, onBack, onNext }: { video: Video; onBack?: () => void; onNext: () => void }) {
  const { guestId, subscribed, mutate, data, player, payment } = useApp();
  ...
  // ── Get active gateway for unlock button ──
  const activeGw = getActiveGateway(payment);

  const handleUnlockClick = () => {
    if (!activeGw) return;
    if (activeGw.type === 'Cashfree' && activeGw.cashfreeLink) {
      window.open(activeGw.cashfreeLink, '_blank');
    }
    // For Razorpay/UPI, redirect user to Plans page
    // (Plans page handles full payment flow)
  ...
  if (locked) return (
    <div ref={wrap} className="relative mx-auto grid h-[78vh] max-h-[820px] min-h-[560px] w-full max-w-[430px] place-items-center overflow-hidden rounded-[34px] bg-zinc-950 text-white shadow-2xl">
      ...
      <div className="relative p-8 text-center">
        ...
        {/* ── FIXED: No external paymentHelper, uses gateway config ── */}
        {activeGw?.type === 'Cashfree' && activeGw.cashfreeLink ? (
          <button
        ...
        ) : activeGw?.type === 'Razorpay' || activeGw?.type === 'UPI Manual' ? (
          <a href="#plans" className="btn mt-5 inline-flex">View Plans</a>
        ) : (
...
function Plans() {
  ...
  // ── FIX: Cashfree handler — opens link in new tab ──
  const handleCashfree = (gw: GatewayConfig) => {
    if (!gw.cashfreeLink) { setPayErr('Cashfree link configure nahi hai. Admin se contact karein.'); return; }
    window.open(gw.cashfreeLink, '_blank');
...
// ── GatewayPayBlock — now supports Cashfree too ──
function GatewayPayBlock({
  gateway, plan, busy, txId, setTxId, payment, onRazorpay, onUpiManual, onCashfree
}: {
  gateway: GatewayConfig; plan: Plan; busy: boolean;
  txId: string; setTxId: (v: string) => void; payment: PaymentSettings;
  onRazorpay: () => void; onUpiManual: () => void;
  onCashfree: () => void; // <-- NEW
}) {
  // ── Cashfree Gateway ──
    ...
    <div className="space-y-3">
      ...
      {gateway.cashfreeLink ? (
        ...
        <div className="rounded-2xl bg-red-50 p-3 text-sm font-bold text-red-700">
          Cashfree link configure nahi hua. Admin panel &gt; Gateways mein link add karein.
        </div>
      )}
    </div>
  );
...
function Profile() {
  ...
  // ── FIX: Save name to localStorage for persistence ──
  const saveProfile = () => {
    localStorage.setItem('rr_display_name', name);
    user && mutate('users', 'PUT', { id: user.id, display_name: name, email }).then(() => setAuthMsg('Profile update ho gaya.'));
  };
...
function PwaInstall() {
  ...
  return <>
    ...
    <button onClick={dismissForever} className="fixed bottom-[4.5rem] left-4 z-40 text-xs text-zinc-400 md:left-auto md:right-6">Dismiss</button>
  </>;
}
...
function Admin() {
  ...
  const addGateway = () => {
    ...
    const newGw: GatewayConfig = {
      ...
      type: gwForm.type,
      enabled: true,
      isDefault: pay.gateways.length === 0,
      testMode: !!gwForm.testMode,
      keys: gwForm.keys || {},
      cashfreeLink: gwForm.cashfreeLink || '',
  ...
  const updateGateway = (id: string, changes: Partial<GatewayConfig>) => {
    const updated = { ...pay, gateways: pay.gateways.map(g => g.id === id ? { ...g, ...changes } : g) };
    setPay(updated);
  };

  const removeGateway = (id: string) => {
    const updated = { ...pay, gateways: pay.gateways.filter(g => g.id !== id) };
    setPay(updated);
  };

  const setDefault = (id: string) => {
    const updated = { ...pay, gateways: pay.gateways.map(g => ({ ...g, isDefault: g.id === id })) };
    setPay(updated);
  };
  ...
  return (
    <main className="min-h-screen bg-orange-50 p-4 md:p-8">
      ...
      {/* ── GATEWAYS TAB — Full gateway engine with Cashfree support ── */}
      {tab === 'gateways' && (
        <div className="space-y-5">
          <div className="panel">
            <h2 className="adminh"><Wallet /> Payment Gateway Engine</h2>
            <p className="text-sm text-zinc-500 mb-4">Razorpay, UPI Manual, aur Cashfree — teeno admin se toggle karein. Default gateway Plans page par use hoga.</p>

            {/* Existing Gateways List */}
            <div className="space-y-3 mb-5">
              {pay.gateways.length === 0 && (
                <div className="rounded-2xl bg-zinc-100 p-4 text-sm text-zinc-500 text-center">Koi gateway nahi hai. Neeche se add karein.</div>
              )}
              {pay.gateways.map(gw => (
                <div key={gw.id} className={`rounded-3xl border-2 p-4 ${gw.enabled ? 'border-green-200 bg-green-50' : 'border-zinc-200 bg-zinc-50'}`}>
                  <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {gw.testMode && <span className="rounded-full bg-yellow-200 px-2 py-0.5 text-xs font-black text-yellow-800">TEST</span>}
                    </div>
                    <div className="flex gap-2">
                  ...
                  {Object.entries(gw.keys || {}).map(([k, v]) => (
                    <p key={k} className="text-xs text-zinc-500 font-mono">{k}: {k.toLowerCase().includes('secret') ? '••••••••' : String(v).slice(0, 20) + (String(v).length > 20 ? '...' : '')}</p>
                  ))}
                  {/* ── Cashfree link inline edit ── */}
                  {gw.type === 'Cashfree' && (
                    <div className="mt-3">
                      <label className="label text-xs">Cashfree Payment Link (editable)</label>
                      <div className="flex gap-2">
                        <input
                          className="input flex-1 text-xs"
                          value={gw.cashfreeLink || ''}
                          onChange={e => updateGateway(gw.id, { cashfreeLink: e.target.value })}
                          placeholder="https://payments.cashfree.com/forms/yourlink"
                        />
                      </div>
                    </div>
                  )}
            ...
            <div className="rounded-3xl border-2 border-dashed border-zinc-300 p-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  ...
                  <input className="input" value={gwForm.name || ''} onChange={e => setGwForm({ ...gwForm, name: e.target.value })} placeholder="Cashfree Production" />
                </div>
                <div>
                  ...
                  <select className="input" value={gwForm.type || ''} onChange={e => setGwForm({ ...gwForm, type: e.target.value, keys: {} })}>
                    ...
                    {['Razorpay', 'Cashfree', 'UPI Manual', 'PayU', 'Paytm', 'PhonePe', 'Stripe', 'PayPal', 'Bank Transfer', 'Other'].map(g => <option key={g}>{g}</option>)}
                  </select>
                </div>
              </div>

              {/* ── Cashfree: only needs a payment link ── */}
              {gwForm.type === 'Cashfree' && (
                <div className="mt-3">
                  <label className="label">Cashfree Payment Page Link *</label>
                  <input
                    className="input"
                    value={gwForm.cashfreeLink || ''}
                    onChange={e => setGwForm({ ...gwForm, cashfreeLink: e.target.value })}
                    placeholder="https://payments.cashfree.com/forms/yourlink"
                  />
                  <p className="text-xs text-zinc-400 mt-1">Cashfree dashboard se Payment Link copy karein aur yahan paste karein.</p>
                </div>
              )}

              {/* ── Razorpay / PayU / Cashfree API keys ── */}
              {(gwForm.type === 'Razorpay' || gwForm.type === 'PayU') && (
                <div className="grid gap-3 md:grid-cols-2 mt-3">
                  ...
                  <div className="md:col-span-2"><label className="label">Webhook Secret (optional)</label><input className="input" type="password" value={gwForm.webhookSecret || ''} onChange={e => setGwForm({ ...gwForm, webhookSecret: e.target.value })} placeholder="Webhook secret" /></div>
                </div>
              )}
              ...
              {(gwForm.type === 'Stripe' || gwForm.type === 'PayPal') && (
                <div className="grid gap-3 md:grid-cols-2 mt-3">
                  <div><label className="label">Publishable / Client Key</label><input className="input" value={gwForm.keys?.keyId || ''} onChange={e => setGwForm({ ...gwForm, keys: { ...gwForm.keys, keyId: e.target.value } })} /></div>
            ...
            <div className="mt-5 space-y-3">
              ...
              <div>
                <label className="label">Payment Instructions (users ko dikhega)</label>
                <textarea className="input min-h-20" value={pay.instructions || ''} onChange={e => setPay({ ...pay, instructions: e.target.value })} placeholder="UPI se payment karein aur UTR WhatsApp par bhejein." />
              </div>
            </div>

            <button className="save w-full mt-4" onClick={() => saveSetting('payment', pay)}>
              💾 Save All Gateway Settings
            </button>

            {/* Status summary */}
            <div className={`mt-4 rounded-2xl p-3 text-sm font-bold flex items-center gap-2 ${pay.gateways.some(g => g.enabled) ? 'bg-green-50 text-green-700' : 'bg-zinc-100 text-zinc-500'}`}>
              {pay.gateways.some(g => g.enabled)
                ...
                : <><span>⏳</span> Koi gateway active nahi — upar se gateway add/enable karein.</>}
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'player' && (
        <div className="panel">
          ...
          <button className="save" onClick={() => saveSetting('player', pl)}>Save Player Settings</button>
        </div>
      )}
