...
function Provider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{refresh()},[]);
  useEffect(()=>{const h=()=>setTimeout(()=>refresh(true),1500);window.addEventListener('supabase-data-updated',h);return()=>window.removeEventListener('supabase-data-updated',h)},[guestId]);
  useEffect(()=>{
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((event:string,session:Session|null)=>{
      if(session?.user?.email){localStorage.setItem('rr_guest',session.user.id);fetch('/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({guest_id:session.user.id,display_name:session.user.user_metadata?.full_name||session.user.email,email:session.user.email,role:'viewer'})}).then(()=>refresh(true))
    });
...
function Player({video,onBack,onNext}:{video:Video;onBack?:()=>void;onNext:()=>void}){
  ...
  const startPlayback=async(full=false)=>{const v=ref.current;if(!v||locked)return;try{v.volume=vol;v.muted=muted;v.playbackRate=speed;await v.play();setPlay(true);if(full||cinema){setCinema(true);await enterFull()}reveal()}catch{setShow(true)}};
  const pausePlayback=()=>{ref.current?.pause();setPlay(false);setShow(true)};
  useEffect(()=>{reveal();resumed.current=false;setErr('');setCur(0);setDur(0);setPlay(false);setCinema(false)},[video.id]);
  ...
  if(player?.mode==='bunny')return <div ref={wrap} className="relative mx-auto h-[78vh] max-h-[820px] min-h-[560px] w-full max-w-[430px] overflow-hidden rounded-[34px] bg-black text-white shadow-2xl transform-gpu" style={{willChange:'transform'}}><iframe src={bunnyIframeUrl(video,player)} title={video.title} allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen className="h-full w-full border-0"/><div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-black/75 to-transparent p-4"><div className="flex items-center gap-3"><button onClick={onBack} className="icon pointer-events-auto"><ArrowLeft/></button><div className="min-w-0"><h2 className="truncate font-black">{video.title}</h2><p className="text-xs text-[var(--rr-primary)]">EP {video.episode_number}</p></div></div></div></div>;
  return <div ref={wrap} onClick={surface} onMouseMove={reveal} className="relative mx-auto h-[78vh] max-h-[820px] min-h-[560px] w-full max-w-[430px] overflow-hidden rounded-[34px] bg-black text-white shadow-2xl transform-gpu select-none" style={{willChange:'transform'}}>
    <video ref={ref} src={vurl(video.video_filename)} poster={video.thumbnail_url} playsInline preload="auto" className="h-full w-full object-cover transform-gpu" onLoadedMetadata={e=>{setDur(e.currentTarget.duration||video.duration_seconds);if(!resumed.current){const h=(data.watch_history||[]).find(w=>w.video_id===video.id);if(h?.current_position)e.currentTarget.currentTime=Number(h.current_position);resumed.current=true}}} onWaiting={buffer} onPlaying={()=>{setWait(false);clearTimeout(buf.current);setPlay(true)}} onCanPlay={()=>{setWait(false);clearTimeout(buf.current)}} onError={()=>{setErr('Unable to load video');setWait(false)}} onPause={()=>setPlay(false)} onEnded={()=>{mutate('video_views','POST',{user_id:guestId,video_id:video.id,watch_seconds:cur,completed:true,device:'web'});onNext()}} onTimeUpdate={e=>{const n=Date.now();if(n-last.current<250)return;last.current=n;setCur(e.currentTarget.currentTime);setDur(e.currentTarget.duration||0);mutate('watch_history','POST',{user_id:guestId,video_id:video.id,current_position:e.currentTarget.currentTime,duration:e.currentTarget.duration||0,completed:false}).catch(()=>{})}}/>
    {!play&&!wait&&!err&&<button onClick={(e)=>{e.stopPropagation();startPlayback(true)}} className="absolute inset-0 z-10 grid place-items-center bg-black/20"><span className="grid h-20 w-20 place-items-center rounded-full bg-white/90 text-black shadow-2xl"><Play size={38} className="translate-x-1"/></span></button>}
    ...
    {err&&<div className="absolute inset-0 z-30 grid place-items-center bg-black/75"><div className="rounded-3xl bg-white p-6 text-center text-zinc-950"><b>{err}</b><button onClick={()=>{setErr('');ref.current?.load();startPlayback(false)}} className="btn mt-4"><RefreshCw/> Retry</button></div></div>}
    ...
    <motion.div animate={{opacity:show?1:0}} transition={{duration:.2}} className="absolute inset-0 z-20 bg-gradient-to-b from-black/75 via-transparent to-black/85">
      ...
      <div className="absolute inset-x-0 bottom-0 space-y-3 p-4"><div ref={bar} onPointerDown={e=>{setDrag(true);seek(e.clientX)}} onPointerMove={e=>drag&&seek(e.clientX)} onPointerUp={e=>{setDrag(false);seek(e.clientX)}} className="h-5 cursor-pointer py-2"><div className="h-1 rounded-full bg-white/20"><div className="relative h-full rounded-full bg-[var(--rr-primary)]" style={{width:`${p}%`}}><span className={`absolute right-0 top-1/2 h-4 w-4 -translate-y-1/2 translate-x-1/2 rounded-full bg-[var(--rr-primary)] ${drag?'opacity-100':'opacity-0'}`}/></div></div></div>
        <div className="flex items-center gap-2"><button className="grid h-11 w-11 place-items-center rounded-full bg-[var(--rr-primary)] text-black" onClick={()=>play?pausePlayback():startPlayback(true)}>{play?<Pause/>:<Play/>}</button><span className="min-w-[84px] text-xs font-bold tabular-nums">{ftime(cur)} / {ftime(dur)}</span><button onClick={()=>jump(-10,'left')} className="rounded-full bg-white/15 px-3 py-2 text-xs font-black">-10</button><button onClick={()=>jump(10,'right')} className="rounded-full bg-white/15 px-3 py-2 text-xs font-black">+10</button><div className="relative ml-auto"><button onClick={()=>setMenu(!menu)} className="rounded-full bg-white/15 px-3 py-2 text-xs font-black">{speed}x</button>{menu&&<div className="absolute bottom-11 right-0 rounded-2xl bg-zinc-950 p-1 shadow-xl">{[.5,.75,1,1.25,1.5,2].map(s=><button key={s} onClick={()=>{setSpeed(s);setMenu(false)}} className="block w-full rounded-xl px-4 py-2 text-left text-sm hover:bg-white/10">{s}x</button>)}</div>}</div><button onClick={()=>{const n=!muted;setMuted(n)}} className="icon h-10 w-10">{muted?<VolumeX/>:<Volume2/>}</button><button onClick={enterFull} className="icon h-10 w-10"><Maximize/></button></div>
      </div>
    </motion.div>
  </div>
}

function HomePage({go}:{go:(t:string)=>void}){const {data,videos,categories}=useApp();const banners=data.banners||[];const hero=banners.find(b=>b.is_active);const heroTitle=hero?.title||'ReelRamp Pro Originals';const heroSub=hero?.subtitle||'Mobile-first short episodes, premium stories aur seamless streaming — sirf aapke liye.';const heroImg=hero?.image_url||fallbackImages.hero;return <section className="space-y-8"><div className="relative overflow-hidden rounded-[40px] bg-zinc-950 text-white shadow-2xl md:grid md:grid-cols-[1.05fr_.95fr]"><div className="absolute -left-24 -top-24 h-64 w-64 rounded-full bg-[var(--rr-primary)]/25 blur-3xl"/><div className="absolute bottom-0 right-1/3 h-52 w-52 rounded-full bg-[var(--rr-accent)]/20 blur-3xl"/><div className="relative p-8 md:p-12"><p className="inline-flex rounded-full bg-white/10 px-4 py-2 text-sm font-black text-[var(--rr-primary)]">ReelRamp Originals</p><h1 className="mt-5 text-5xl font-black leading-tight md:text-7xl">{heroTitle}</h1><p className="mt-5 max-w-xl text-lg text-white/75">{heroSub}</p><div className="mt-7 flex flex-wrap gap-3"><button onClick={()=>go('forYou')} className="btn">{hero?.cta_label||'Start Watching'}</button><button onClick={()=>go('plans')} className="rounded-full border border-white/15 bg-white/10 px-6 py-3 font-black text-white">View Plans</button></div><div className="mt-8 grid grid-cols-3 gap-3 max-w-md"><div className="rounded-3xl bg-white/10 p-4"><b>{videos.length}+</b><small className="block text-white/60">Episodes</small></div><div className="rounded-3xl bg-white/10 p-4"><b>{categories.length}</b><small className="block text-white/60">Categories</small></div><div className="rounded-3xl bg-white/10 p-4"><b>HD</b><small className="block text-white/60">Bunny CDN</small></div></div></div><div className="relative min-h-[420px]"><img src={heroImg} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-transparent to-transparent md:bg-gradient-to-r md:from-zinc-950 md:to-transparent"/><div className="absolute bottom-5 left-5 right-5 rounded-3xl bg-black/45 p-4 text-white backdrop-blur"><p className="text-sm font-black text-[var(--rr-primary)]">Featured Premiere</p><h3 className="text-2xl font-black">Stories jo sirf sunayi nahi, mehsoos hoti hain.</h3></div></div></div><Rows title="Categories">{categories.filter(c=>c.is_active).map(c=><button key={c.id} onClick={()=>go('forYou')} className="card shrink-0 px-5 py-4 font-black transition hover:-translate-y-1 hover:shadow-xl"><span className="mr-2">{c.icon}</span>{c.name}</button>)}</Rows><Rows title="Trending Stories">{videos.filter(v=>v.is_published).slice(0,10).map(v=><VideoCard key={v.id} v={v} onClick={()=>go('forYou')}/>)}</Rows><div className="grid gap-4 md:grid-cols-3"><FeatureCard icon="🎬" title="Original Shorts" body="Drama, romance, thriller aur family stories ek mobile-first format me."/><FeatureCard icon="👑" title="Premium Unlock" body="Admin-controlled plans, paywall aur Razorpay-ready payment structure."/><FeatureCard icon="📲" title="Install App" body="PWA install se app jaisa home-screen experience paayein."/></div><Info/></section>}
...
function VideoCard({v,onClick}:{v:Video;onClick:()=>void}){return <button onClick={onClick} className="card group w-56 shrink-0 overflow-hidden text-left transition hover:-translate-y-1 hover:shadow-2xl"><div className="relative aspect-[3/4] bg-zinc-200">{v.thumbnail_url?<img src={v.thumbnail_url} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/>:<img src={fallbackImages.promo} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/>}<div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent"/><span className="absolute left-3 top-3 rounded-full bg-black/55 px-3 py-1 text-xs font-black text-white backdrop-blur">EP {v.episode_number}</span>{v.is_premium&&<span className="absolute right-3 top-3 rounded-full bg-yellow-300 px-3 py-1 text-xs font-black text-black">PRO</span>}<span className="absolute bottom-3 left-3 grid h-11 w-11 place-items-center rounded-full bg-white text-black shadow-xl"><Play size={18}/></span></div><div className="p-4"><p className="text-xs font-black text-[var(--rr-accent)]">{v.category} · {v.series_title}</p><h3 className="line-clamp-2 text-lg font-black">{v.title}</h3><p className="mt-1 line-clamp-2 text-xs text-zinc-500">{v.description}</p></div></button>}

function ForYou(){const {videos,categories,mutate,guestId}=useApp();const [cat,setCat]=useState('All'),[idx,setIdx]=useState(0),[touch,setTouch]=useState<number|null>(null),[report,setReport]=useState('');const list=videos.filter(v=>v.is_published&&(cat==='All'||v.category===cat));const v=list[idx]||list[0];useEffect(()=>setIdx(0),[cat]);if(!v)return <Empty/>;const next=()=>setIdx(i=>list.length?(i+1)%list.length:0),prev=()=>setIdx(i=>list.length?(i-1+list.length)%list.length:0);return <section className="mx-auto w-full max-w-6xl overflow-x-hidden"><div className="mb-4 flex gap-2 overflow-x-auto pb-1">{['All',...categories.filter(c=>c.is_active).map(c=>c.name)].map(c=><button key={c} onClick={()=>setCat(c)} className={`pill ${cat===c?'active':''}`}>{c}</button>)}</div><div className="grid items-start justify-center gap-5 lg:grid-cols-[minmax(320px,430px)_minmax(280px,420px)]"><div className="w-full" onWheel={e=>{if(Math.abs(e.deltaY)>30){e.preventDefault();e.deltaY>0?next():prev()}}} onTouchStart={e=>setTouch(e.touches[0].clientY)} onTouchEnd={e=>{if(touch===null)return;const diff=touch-e.changedTouches[0].clientY;if(Math.abs(diff)>55){diff>0?next():prev()}setTouch(null)}}><Player video={v} onNext={next}/></div><aside className="w-full max-w-[430px] space-y-4 lg:sticky lg:top-32"><div className="card p-5"><p className="font-black text-[var(--rr-accent)]">{v.series_title}</p><h1 className="text-3xl font-black leading-tight">{v.title}</h1><p className="mt-3 text-sm text-zinc-600">{v.description}</p><div className="mt-4 rounded-2xl bg-zinc-100 p-3 text-sm font-bold">Swipe up/down ya next/previous se episodes navigate karein.</div><div className="mt-4 flex gap-2"><button className="pill" onClick={prev}>Previous</button><button className="pill active" onClick={next}>Next</button></div></div><div className="max-h-72 space-y-2 overflow-y-auto pr-1">{list.map((e,i)=><button key={e.id} onClick={()=>setIdx(i)} className={`w-full rounded-[22px] p-3 text-left text-sm font-bold ${i===idx?'bg-zinc-950 text-white':'bg-white shadow-sm'}`}>EP {e.episode_number}: {e.title}</button>)}</div><div className="card p-4"><h3 className="font-black">Report content</h3><input className="input" value={report} onChange={e=>setReport(e.target.value)} placeholder="Reason"/><button className="btn w-full" onClick={()=>mutate('content_reports','POST',{user_id:guestId,video_id:v.id,reason:report||'User report',details:report,status:'open'}).then(()=>setReport(''))}>Submit Report</button></div></aside></div></section>}

function Plans(){
  const {plans,payment,guestId,mutate,subscribed}=useApp();
  const activate=async(p:Plan)=>{
    await mutate('payments','POST',{user_id:guestId,plan_id:p.id,amount:p.price,gateway:payment.gateway,status:'success',notes:'Activated'});
    await mutate('subscriptions','POST',{user_id:guestId,plan:p.name,status:'active',expires_at:new Date(Date.now()+p.duration_days*86400000).toISOString()})
  };
  return <section id="plans" className="space-y-5">
    <Title t="Plans" s="Premium plans admin panel se manage hote hain."/>
    {subscribed&&<div className="rounded-3xl bg-green-100 p-4 font-black text-green-800">Premium Active ✅</div>}
    <div className="grid gap-4 md:grid-cols-3">{plans.filter(p=>p.is_active).map(p=><div key={p.id} className="card p-6"><Crown className="text-yellow-500"/><h3 className="mt-3 text-2xl font-black">{p.name}</h3><p className="text-4xl font-black">{money(p.price)}</p><p className="text-zinc-500">{p.duration_days} days</p><button onClick={()=>activate(p)} className="btn mt-5 w-full">Activate</button></div>)}</div>
    <div className="card p-5"><b>Payment Gateway:</b><p>{payment.gateway}{payment.upiId?` · UPI: ${payment.upiId}`:''}{payment.whatsapp?` · WhatsApp: ${payment.whatsapp}`:''}</p></div>
  </section>
}
...
function Profile(){
  ...
  const signIn=async(signUp=false)=>{
    ...
    else setAuthMsg(signUp?'Account ban gaya! Agar email confirmation on hai to inbox check karein.':'Login successful! Welcome back.')
  };
  const signOut=async()=>{await supabase.auth.signOut();localStorage.removeItem('rr_guest');location.reload()};
  ...
  return <section className="space-y-6">
    <div className="overflow-hidden rounded-[36px] bg-zinc-950 text-white shadow-2xl md:grid md:grid-cols-[1.05fr_.95fr]">
      <div className="relative p-7 md:p-10"><div className="absolute -left-16 -top-16 h-44 w-44 rounded-full bg-[var(--rr-primary)]/25 blur-3xl"/>
        ...
        <p className="relative mt-4 text-white/70">Email, Google ya fast guest profile — account active hai, plan status aur profile sync hoga.</p>
      ...
      <div className="bg-white p-6 text-zinc-950 md:p-8">
        ...
        {isLoggedIn&&<>
          <input className="input" value={name} onChange={e=>setName(e.target.value)} placeholder="Display name"/>
          <input className="input" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email address"/>
          <div className="flex flex-wrap gap-2"><button className="btn" onClick={saveProfile}>Save Profile</button><button className="rounded-full bg-zinc-200 px-5 py-3 font-bold" onClick={signOut}>Logout</button></div>
        </>}
...
function Info(){return <div className="card overflow-hidden md:grid md:grid-cols-[.9fr_1.1fr]"><img src={fallbackImages.studio} className="h-full min-h-72 w-full object-cover"/><div className="p-7"><p className="font-black text-[var(--rr-accent)]">About ReelRamp</p><h2 className="mt-2 text-3xl font-black">ReelRamp Originals Pvt. Ltd.</h2><p className="mt-3 text-zinc-600">Founder/Director Ayush Jivan — "Kahaniyan sirf sunayi nahi jati, mehsoos ki jati hain."</p><p className="mt-4 text-zinc-600">FF Shop No. 6, Arohi Arcade, Munshipulia, Lucknow - 226016 · reelramporiginal@gmail.com · +91 7307493338</p><div className="mt-5 flex gap-3 text-[var(--rr-accent)]"><Facebook/><Instagram/><Youtube/><MessageCircle/></div></div></div>}
