  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((event:string,session:Session|null)=>{
      if(session?.user?.email){
        ...
        // FIX: location.reload() hataaya — ab sirf refresh(true) hoga, blink nahi hoga
        .then(()=>refresh(true));
      }
    });
...
// FIX: PWA Install — direct install, Chrome ka "Add to home screen" popup nahi
function PwaInstall(){
  ...
  useEffect(()=>{
    ...
    // beforeinstallprompt capture karo — yahi real install event hai
    ...
    // 4 second baad popup dikha agar install nahi hua
  ...
  const install=async()=>{
    setInstalling(true);
    if(deferred){
      // Chrome/Android — direct install prompt
      deferred.prompt();
      ...
      // iOS ke liye instructions
      alert('iPhone par:\n1. Neeche Share button (box with arrow) dabao\n2. "Add to Home Screen" select karo\n3. "Add" dabao');
    } else {
      // Fallback — browser menu se
      alert('Browser ke menu (3 dots) mein jaao → "Install App" ya "Add to Home Screen" select karo');
    }
    setInstalling(false);
  };
